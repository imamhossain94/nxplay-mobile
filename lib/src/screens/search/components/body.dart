...
import 'package:nx_play/src/bloc/get_search_video_response_bloc.dart';
...
class _BodyState extends State<Body> {
  ...
  void initState() {
    videosList = [];
    searchVideoResponseBloc..getSearchVideos(widget.params);
    super.initState();
  }

  @override
  void dispose() {
    searchVideoResponseBloc..drainStream();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<GetSearchVideosResponse>(
      stream: searchVideoResponseBloc.subject.stream,
      builder: (context, AsyncSnapshot<GetSearchVideosResponse> snapshot) {
        if (snapshot.hasData) {
  ...
  Widget _buildMoreVideosWidget(GetSearchVideosResponse data) {
    //videosList.clear();
    ...
    return SafeArea(
      child: Scaffold(
        ...
        body: Stack(
          children: [
            videosList.length != 0? _paginationView(data.getMoreVideos):Center(child: Text('Empty search result'),),
